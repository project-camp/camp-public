#' Comprehensive Geostatistical Workflow for Compositional Data
#'
#' @description
#' Performs complete geostatistical analysis including variogram modeling, 
#' cokriging prediction, visualization, and validation for compositional data.
#' Supports both omnidirectional and directional approaches with MAF transformations.
#'
...
#' 1. Creates method-specific output directories
#' 2. Variogram modeling (omnidirectional or directional)
#' 3. Linear Model of Coregionalization (LMC) fitting
#' 4. Cokriging prediction on regular grid
#' 5. Map visualization and export
#' 6. Cross-validation and spatial validation
#' 
#' Output structure:
#' - RDS files saved to: ./test/ck/[method-name]/
#' - Maps saved to: ./test/ck/[method-name]/maps/
#' 
#' @author Antonios Koutroumpas, Abel Ruiz-Giralt
#'
#' @examples
#' \dontrun{
#' # Basic omnidirectional workflow
#' results <- geostatistical_workflow(spdf_ilr, "ILR_Omnidirectional", 
#'                                   lag_dist, site_diag, directional = FALSE)
#' 
#' # MAF-based workflow
#' results <- geostatistical_workflow(spdf_maf, "MAF_Omnidirectional", 
#'                                   lag_dist, site_diag, directional = FALSE,
#'                                   maf_result = maf_result, resolution = 2)
#' }
#'
#' @export

geostatistical_workflow <- function(spdf_data, method_name, lag_dist, site_diag, 
  ...
  ck_cross_validation(spdf_data, g_krig, nfold = 5, plot_scatter = TRUE, plot_hist = TRUE, plot_qq = TRUE)
  
  par(bg = "white")
  options(repr.plot.width = 9, repr.plot.height = 9)
  source("./utils/functions/ck_spatial_validation.R")
  ck_spatial_validation(spdf_data, ck, accuracy_plot = TRUE)
